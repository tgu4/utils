...
getConfiguredClient() {
  if command -v curl &>/dev/null; then
    ...
    echo "Error: This tool requires either curl, wget, \
    httpie or fetch to be installed." >&2
    return 1
  fi
}
...
usage() {
  cat <<EOF
Geo
Description: Provides quick access for wan, lan, router, dns, mac, and ip geolocation \
data
Usage: geo [flag]
  -w  Returns WAN IP
  -l  Returns LAN IP(s)
...
update() {
  ...
  if [[ $currentVersion == "" || $repositoryName == "" || $githubUserName == "" ||
    $nameOfInstallFile == "" ]]; then
    ...
    if [[ "$latestVersion" != "$currentVersion" ]]; then
      ...
      if [[ "$answer" == [Yy] ]]; then
        ...
        if [[ -d ~/$repositoryName ]]; then
          rm -r -f $repositoryName || {
            echo "Permissions Error: try running the update as sudo"
            exit 1
          }
        fi
        echo -n "Downloading latest version of: $repositoryName."
        git clone -q "https://github.com/$githubUserName/$repositoryName" &&
          touch .BSnippetsHiddenFile || {
          echo "Failure!"
          exit 1
        } &
        while [ ! -f .BSnippetsHiddenFile ]; do {
          echo -n "."
          sleep 2
        }; done
        ...
        git checkout "v$latestVersion" 2>/dev/null ||
          ...
          echo "Couldn't git checkout to stable release, updating to latest commit."
        chmod a+x install.sh #this might be necessary in your case but wasnt in mine.
        ./$nameOfInstallFile "update" || exit 1
        cd ..
        rm -r -f $repositoryName || {
          echo "Permissions Error: update succesfull but cannot delete temp files \
          located at ~/$repositoryName delete this directory with sudo"
          exit 1
        }
      else
        exit 1
